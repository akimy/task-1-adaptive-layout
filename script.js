...
document.addEventListener("DOMContentLoaded", () => {
    ...
    data.forEach((card) => {
        ...
        switch(size) {
            case 's': {
                const clone = smallTemplate.content.cloneNode(true);
                const labelNode = clone.querySelector('.small__label');
                const imageNode = clone.querySelector('.small__image');
                const descriptionNode = clone.querySelector('.small__description');
                if (image) {
                    const imageId = image.match(/\/\d+/g);
                    imageNode.setAttribute('src', `assets/images${imageId}@3x.png`);
                    descriptionNode.remove();
                } else {
                    labelNode.style.position = 'initial';
            ...
            case 'm': {
                ...
                const labelNode = clone.querySelector('.medium__label');
                const imageNode = clone.querySelector('.medium__image');
                const descriptionNode = clone.querySelector('.medium__description-text');
                const imageId = image.match(/\/\d+/g);
                imageNode.setAttribute('src', `assets/images${imageId}@3x.png`);
                labelNode.innerHTML = title;
                labelNode.style.color = titleColor;
            ...
            case 'l': {
                const clone = largeTemplate.content.cloneNode(true);
                const labelNode = clone.querySelector('.large__label');
                const imageNode = clone.querySelector('.large__image');
                const descriptionNode = clone.querySelector('.large__description');
                const imageId = image.match(/\/\d+/g);
                imageNode.setAttribute('src', `assets/images${imageId}@3x.png`);
                labelNode.innerHTML = title;
                labelNode.style.color = titleColor;
